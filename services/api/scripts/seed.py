"""Seed script for local development and Render demo deploy."""
...
async def _ensure_users(session) -> tuple[User, User, User | None]:
...
async def _seed_demo_content(session, *, user: User, admin: User) -> None:
    ...
    for article in articles:
        existing = await session.execute(select(FeedArticle).where(FeedArticle.slug == article["slug"]))
    ...
    profile_result = await session.execute(select(KworkProfile).where(KworkProfile.user_id == user.id))
    ...
    room_result = await session.execute(select(ChatRoom).where(ChatRoom.name == DEMO_CHAT_ROOM_NAME))
    ...
    msg_result = await session.execute(select(ChatMessage).where(ChatMessage.room_id == room.id).limit(1))
...
async def seed() -> None:
    async with SessionLocal() as session:
        user, admin, _moderator = await _ensure_users(session)
        print("Seed: demo users ready (artist/admin/moderator@example.com)", flush=True)

    async with SessionLocal() as session:
        user_result = await session.execute(select(User).where(User.email == "artist@example.com"))
        user = user_result.scalar_one()
        admin_result = await session.execute(select(User).where(User.email == "admin@example.com"))
        admin = admin_result.scalar_one()
        try:
            await _seed_demo_content(session, user=user, admin=admin)
            print("Seed complete: feed, kworks, office project, chat room, demo chart", flush=True)
        except Exception as exc:
            await session.rollback()
            print(f"Seed: demo content failed (login still works): {exc}", flush=True)
            traceback.print_exc()
